const fs = require('fs');
const depthDetector = require('../../../../src/exercises/advent-of-code-2021/day-1-depth-detector/depth-detector');
...
describe('Advent of Code 2021 - Day 1', () => {
    ...
    it('should calculate the number of times an increase occurs in a small list of numbers', async () => {
        ...
        const result = depthDetector.calculateNumIncreases(depths);
    ...
    it('should calculate the number of times an increase occurs between subsets in a list of numbers', async () => {
        const depths = await fs.readFileSync(inputFile, 'utf-8').split('\n').map((line) => Number(line));
        const start = new Date();
        const result = depthDetector.calculateNumIncreases(depths, 3);
        expect(result).to.not.be.null;
        console.log(`${result}: ${new Date() - start} ms`);
    });
});