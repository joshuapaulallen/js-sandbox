...
describe('bag rule set', () => {
    ...
    describe('bag rule', () => {
       ...
       it('should indicate whether a bag can directly contain a certain number of a given bag type', () => {
           const bagRuleStr = 'dark olive bags contain 3 faded blue bags, 1 dotted black bag.';
           const bagRule = BagRule.parse(bagRuleStr);

           expect(bagRule.calculateCapacity(new BagType('blue', 'faded'))).to.equal(3);
           expect(bagRule.calculateCapacity(new BagType('black', 'dotted'))).to.equal(1);
           expect(bagRule.calculateCapacity(new BagType('red', 'dotted'))).to.equal(0);
       });
    });
    ...
    describe('bag rule set', () => {
        it('should indicate whether a bag can eventually contain a bag of a given bag type for a limited rule set', () => {
            ...
            expect(bagRuleSet.calculateBagCapacity(new BagType('red', 'light'), new BagType('white', 'bright'))).to.equal(1);
            expect(bagRuleSet.calculateBagCapacity(new BagType('red', 'light'), new BagType('yellow', 'muted'))).to.equal(2);
            expect(bagRuleSet.calculateBagCapacity(new BagType('red', 'light'), new BagType('chartreuse', 'muted'))).to.equal(0);
            expect(bagRuleSet.calculateBagCapacity(new BagType('blue', 'faded'), new BagType('black', 'dotted'))).to.equal(0);

            // tests for "indirect" relationships (i.e., bag A contains bag B contains bag C)
            expect(bagRuleSet.calculateBagCapacity(new BagType('red', 'light'), new BagType('gold', 'shiny'))).to.equal((1 * 1) + (2 * 2));
            ...
            const canContainShinyGoldBag = bagRuleSet.getBagTypes()
                .filter((bagType) => bagRuleSet.calculateBagCapacity(bagType, new BagType('gold', 'shiny')) > 0)
